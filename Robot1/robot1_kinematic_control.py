...
class Server:
	def __init__(self):
		...
		self.goal_pos = array([[320.*pi/180., 0.*pi/180., 15.*pi/180.]]).T
		self.skill = 0
		self.lastcm = 0
		self.diff = 0
		self.ball = 0
		...
		self.detect = 0
		self.kick = 0
		print "kinematic robot1 control running"
		print self.skill
	...
	def get_robot2_camera(self, dat):
		if(dat.z != 0):
			self.detect = 1
		else:
			self.detect = 0
...
if __name__ == "__main__":
	...
	pub1 = rospy.Publisher("robot1/kick_ball", Int32, queue_size=2)
	pub2 = rospy.Publisher("robot1/command_clear", Int32, queue_size=2)
	pwm = Vector3()
	er = Float32()
	odom = Vector3()
	tendang = Int32()
